...
public:
  ...
  AudioProgram(const RenderJobVector &jobs, vector<int> generatorIndices)
      : jobs(jobs), generatorIndices(generatorIndices),
        finished(new atomic<int>[jobs.size()]) {
    reset();
  }
  ...
  void run(AudioUnitRenderActionFlags *actionFlags,
    ...
    auto exec = [&](int index) {
      ...
      for (int outputIndex : job->outputIndices) {
        if (finished[outputIndex].fetch_add(1, std::memory_order_relaxed) ==
            jobs[outputIndex]->inputCount()) {
