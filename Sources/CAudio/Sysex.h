...
template <typename T> std::vector<uint8_t> encodeSysex(T value) {
...
///
/// We need this function because event.pointee.MIDI.data is just a tuple of
/// three midi bytes. This is fine for simple midi messages like note on/off,
/// but some messages are longer, so we need access to the full array, which
/// extends off the end of the structure (one of those variable-length C
/// structs).
///
/// - Parameters:
...
void withMidiData(const AUMIDIEvent *event, void (*f)(const uint8_t *)) {
  ...
  intptr_t offset = reinterpret_cast<intptr_t>(&(event->data)) -
...
template <typename T> void decodeSysex(const AUMIDIEvent *event, T &value) {
