...
public:
  ...
  RenderJob(std::shared_ptr<SynrchonizedAudioBufferList2> outputBuffer,
            AURenderBlock renderBlock, AURenderPullInputBlock inputBlock,
            std::vector<int> inputIndices)
      : outputBuffer(outputBuffer), renderBlock(renderBlock),
        inputBlock(inputBlock), inputIndices(inputIndices) {}

  /// Number of inputs feeding this AU.
  int32_t inputCount() { return static_cast<int32_t>(inputIndices.size()); }
  ...
  void render(AudioUnitRenderActionFlags *actionFlags,
    ...
    AudioBufferList *out =
        outputBufferList ? outputBufferList : outputBuffer->abl;
    ...
    AUAudioUnitStatus status =
        renderBlock(actionFlags, timeStamp, frameCount, 0, out, inputBlock);
