...
public:
  ...
  WorkerThread(int index,
      ...
      : workerIndex(index), runQueues(queues), prod(prodSemaphore),
        done(doneSemaphore), workgroup(workgroupInstance) {
  ...
  void main() {
    if (workgroup != nullptr) {
      ...
      /// I'm unsure what to do with the int that is returned, perhaps we can
      /// detect any errors with it?
    ...
    while (true) {
      ...
      if (program) {
        program->run(&actionFlags, &timeStamp, frameCount, outputBufferList,
                     workerIndex, runQueues);
      } else {
